  ...
  if (job && job.type in self.definitions) {
    ...
    fn(job, function(err) {
      if ( err && job.retries < job.retryLimit ) {
        ...
        self.emit('success', err, job);
        self.emit('success:' + job.type, err, job);
      }
    });
  }
});

Jobby.prototype.on('retry', function(err, job) {
  var self = this;
  var query = 'update ' + this._dbTable + ' set retries = retries + 1, status=\'pending\' where id = ' + job.id;
  this.query(query, function(err) {
    if (err) self.emit('job:error', err);
  });
});
